...
## 当前版本：contextual_v2（2026-09-13）

`--dpr` 现在选择 `--dpr_variant contextual_v2`，DPR 本身仍默认关闭。
`--dpr_variant legacy_v1` 保留旧目标作为历史参考；没有版本字段的旧 sidecar
按 legacy_v1 读取，自动恢复拒绝静默切换目标。

v2 保留 v1 的粒子预测、零初始化回写、全部 token 层、CED evidence 和 pre_mix
语义。辅助目标来自本次前向在 DPR 注入前的规范化 CED 状态 a：

```text
P = fixed orthonormal projection [r,d], saved FP32, frozen
u_t = mean_{j=1..k} P * stop_gradient(a_{t+j})
y_t = stop_gradient(sqrt(3) * tanh(u_t / sqrt(mean(u_t^2)+1e-6)))
L = NTP + lambda * mean_valid(kernel_score(pi,z,y)) + existing MoE auxiliary
```

先投影再池化，窗口操作在 r 维进行。目标网络被移除，不增加 teacher 前向。
投影初始化使用独立 NumPy seed，写入 checkpoint，不参与训练或主干重新初始化。
整个目标 stop-gradient；预测支路的历史输入仍可向编码器传梯度。目标状态包含
各自可见的历史上下文，并非 v1 的纯未来 embedding 编码；未来信息仍仅在标签路径。
PAD、EOS、segment、完整窗口与 N=0 的规则沿用 v1。协方差仅作为诊断指标，
不再加进优化目标。`mse` 对照同样使用 detached contextual target，不加协方差正则。

默认 d=1024/M=4/r=32/k=4：可训练 DPR 参数 270,468；含固定投影共 303,236，
旧 v1 共 799,012。T=1024 的分支主要训练 GEMM 估计从 3,735,456 降至
1,687,552 FLOPs/token，约减少 54.8%。这只计算 DPR 分支，不是整模型实测提速。
推理函数没有新增目标计算。相同 retained weights 下 v1/v2 logits 一致。

### 两项数值修复

见 [稳定性审计](DPR_STABILITY_20260913.md)。Adam m/v 和更新计算使用 FP32；
MuonH 改为 checkpoint 持久 FP32 半径，投影在 FP32 完成后才转换参数 dtype。
旧 checkpoint 的 Muon 半径首次按当前矩阵采用，不擅自把训练权重缩回初始化。
梯度统计只复用原来每张量一次的平方和扫描，额外传回标量；DPR JSONL 与 SwanLab
记录 `grad/dpr_predict`、`grad/dpr_output`、`grad/attention`、`grad/router` 等。
范数 >=10 时控制台打印最大梯度张量名称。裁剪使用同一 FP32 统计，日志保留裁剪前范数。

### 保留训练进度的迁移

```bash
# 检查来源、结构、目标目录和保存步数；不写文件
.venv/bin/python scripts/upgrade_dpr_checkpoint.py \
  research_runs/viby_dpr_jepa_v1/pretrain_1024.safetensors \
  research_runs/viby_dpr_jepa_v2 --dry-run

# 确定性转换：新目录必须不存在，原始文件不修改
.venv/bin/python scripts/upgrade_dpr_checkpoint.py \
  research_runs/viby_dpr_jepa_v1/pretrain_1024.safetensors \
  research_runs/viby_dpr_jepa_v2
```

迁移按块复制 tensor 原始字节，只删除旧 target 参数与相应 optimizer state，加入
固定投影。保留优化器时钟、LR 超参、epoch/step、RNG、有效 token 计数和升温预算；
新目录不复制旧 SwanLab run ID。源 sidecar 保存在 `source_metadata.json`。
继续使用原来的数据、batch、累积、packing、LR horizon 配置并指定新目录。
不使用 `--reset_optimizer`，它会重置进度。当前本地源保存于 microstep 9999，
不等于日志结束的 16878，也不包含用户报告的 grad_norm=40 时刻。

### 验证边界

后续[全程审计](DPR_HEALTH_AUDIT_20260913.md)发现 batch 变化后的样本重放与目标分布变窄。
已加入恢复布局校验、`target_mean_square` / `target_second_moment` /
`target_effective_rank` 诊断、最大梯度张量名称及 checkpoint 保存耗时。
这些新增诊断不改变辅助目标；旧 `cov_rank_deficient` 仅检查 N<=r，不能当实际秩判据。
预训练的最后不足累积窗口现在也会正确归一并更新；已完成检查点没有回写修改。

检查覆盖 FP32/BF16、eager/compiled、目标梯度隔离、目标替换不影响 logits、
prefill/decode、packed 文档隔离、空/单目标窗口、共同初始化/RNG、真实 compiled
trainer 累积/QB 更新，以及迁移后的权重/状态/进度/输出一致与继续更新。
Muon 检查覆盖矩阵、逐头、逐专家、BF16 多次更新、零半径和半径保存恢复。
本轮 95 项针对性检查通过（5.01 秒），迁移维度校验补充后单项复查通过；静态检查通过。
原始范数样本、命令与源码快照位于 `research_runs/dpr_contextual_v2_20260913/`。
没有启动长训练、参数搜索或性能 benchmark；未建立 token efficiency 收益，
也没有重放用户报告的 40 范数现场。实现消除了已确认的机制缺陷。

## 以下为 legacy_v1 历史记录（保留当时配置与证据）
