...
Enabled by default after same-cache decode ABBA and exact-logit verification;
VIBY_MOE_DECODE_COMPILE=0 restores the eager region.
"""
...
_ENABLED = os.environ.get("VIBY_MOE_DECODE_COMPILE", "1") != "0"
...
def _compiled_region(n_experts, routes, limit, shared_limit):
    ...
    def forward(x, idx, w, gate_up_w, down_w, shared_gate, shared_down, shared_up):
        ...
        shared = (
            expert_act(x @ shared_gate.T, x @ shared_up.T, shared_limit) @ shared_down.T
        )
        return mixed + shared
