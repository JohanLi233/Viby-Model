"""稠密 GLU（SiLU/SiTU）的 decode 融合 Metal kernel（仅推理前向，小批量）。

h = glu(g, u) → out = h @ W_downᵀ 的两段合成 1 个 kernel。
`hidden_act='silu'` 走 silu(g)·u；`'situ'` 走 β 软帽的 SiTU-GLU。
...
def _build(D: int, I: int, dtype, hidden_act: str):  # noqa: E741
    key = (D, I, dtype, hidden_act)
    ...
    kern = mx.fast.metal_kernel(
        name=f"swiglu_down_{hidden_act}_{D}_{I}_{mt}",
        input_names=["gu", "wd"],
        output_names=["out"],
        source=f"""
        constexpr uint D = {D};
...
def silu_mul_down_decode(gu, wd, hidden_act: str = "silu"):
    """gu (M, 2I) bf16（gate|up 拼接 GEMM 输出），wd (D, I) down 权重。
    hidden_act='silu'（默认）走 SiLU-GLU，否则走 SiTU-GLU。
    ...
    if wd.shape != (D, I) or M > _MAX_ROWS:
        return None
    key = (D, I, gu.dtype, hidden_act)
    try:
        kern = _build(D, I, gu.dtype, hidden_act)
        (out,) = kern(
            inputs=[gu, wd],
            output_shapes=[(M, D)],
        ...
        if key not in _VERIFIED:
            from ..acts import silu_glu, situ_glu

            g, u = mx.split(gu, 2, axis=-1)
            ref = (silu_glu if hidden_act == "silu" else situ_glu)(g, u) @ wd.T
