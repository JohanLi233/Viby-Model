...
def _count_kernel():
    return mx.fast.metal_kernel(
        ...
        input_names=["indices"], output_names=["partial"],
        source=r"""
...
def sequence_route_counts(indices, batch_size, seq_len, n_experts):
    ...
    if mx.default_device() == mx.gpu and mx.metal.is_available() and e <= 512:
        ...
        (partial,) = _count_kernel()(
            inputs=[ids],
            template=[("E", e), ("NT", _THREADS), ("RT", _ROUTES_PER_TILE),
                      ("R", t * k), ("TILES", tiles)],
            grid=(tiles * _THREADS, b, 1), threadgroup=(_THREADS, 1, 1),
            output_shapes=[(b, tiles, e)], output_dtypes=[mx.uint32],
        )
    ...
    counts = mx.zeros((b * e,), dtype=mx.float32).at[flat].add(
        mx.ones((indices.size,), dtype=mx.float32)
    )
