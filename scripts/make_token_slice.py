...
with open(src, "rb") as f, open(dst, "w", encoding="utf-8") as out:
    for line in f:
        ...
        try:
            t = (
                len(
                    tok(json.loads(text)["text"], add_special_tokens=False)["input_ids"]
                )
                + 2
            )
        except Exception:
        ...
        if docs % 100000 == 0:
            print(
                f"{docs} docs, {count / 1e6:.1f}M tokens, {time.time() - t0:.0f}s",
                flush=True,
            )
print(
    f"done: {docs} docs, {count / 1e6:.2f}M tokens -> {dst} ({time.time() - t0:.0f}s)"
)
