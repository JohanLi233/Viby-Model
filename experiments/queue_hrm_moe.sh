...
# + CycleRouter（cycle_emb 按迭代特化路由，破循环引理包含上界）
...
# CycleRouter 无关（关 cycle_emb 对照臂同样塌）。修复：bias 0.005 +
...
./experiments/run_exp.sh r070_hrm_moe \
  ...
  --moe_intermediate_size 104 --n_dense_layers 0 --routed_scaling_factor 2.5 \
  --hrm_H_cycles 2 --hrm_L_cycles 3 --hrm_bp_cycles 2 \
  --hrm_emb_scale 27.7128 --hrm_cycle_router 1 --hrm_cycle_film 1 \
