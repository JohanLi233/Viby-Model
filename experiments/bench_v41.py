...
def main():
    ...
    ap.add_argument("--experts", type=int, default=0, help="覆盖路由专家数（0 = 用预设）")
    ap.add_argument("--grad", action="store_true", help="额外测 fwd+bwd")
    ap.add_argument("--no-mtp", action="store_true")
    ap.add_argument("--parity", action="store_true", help="额外跑 prefill/decode 一致性")
    args = ap.parse_args()
    ...
    print(
        ...
        % (
            cfg.dim, cfg.n_layers, cfg.n_encoder_layers, cfg.n_heads, cfg.head_dim,
            cfg.window_size, cfg.hc_mult, cfg.n_activated_experts, cfg.n_routed_experts,
        )
    )
    modes = {}
    ...
    print("kv 源:", list(cfg.kv_source_layers), " indexer 源:", list(cfg.index_source_layers),
          " 候选层:", cfg.candidate_source_layer)

    t0 = time.time()
    model = VibyForCausalLM(cfg)
    print("构建 %.1fs  总参 %.1fM  激活 %.1fM  Engram 表 %.1fM"
          % (time.time() - t0, model.num_parameters() / 1e6,
             cfg.num_active_parameters() / 1e6, model.ngram_lookup_parameters() / 1e6))
    ...
    mx.eval(out.loss)
    dt = time.time() - t0
    print("fwd   B=%d T=%d  %.3fs  %.0f tok/s  peak %.2f GB  loss %.3f (lm %.3f mtp %.3f)"
          % (B, T, dt, B * T / dt, mx.get_peak_memory() / 1e9, out.loss.item(),
             out.lm_loss.item(), (out.mtp_loss or mx.array(0.0)).item()))
    if out.moe_loads is not None:
        print("      MoE 负载：每层计数和 %s（= B*T*top_k）"
    ...
    if args.grad:
        ...
        mx.eval(loss, grads)
        dt = time.time() - t0
        print("f+bwd B=%d T=%d  %.3fs  %.0f tok/s  peak %.2f GB"
    ...
    if args.parity:
        ...
        print("prefill/decode 一致性 max|Δlogit| = %.3e（fp32 噪声级 ≈1e-6 为合格）" % d)
