...
def main():
    ...
    seg = mx.cumsum(
        (mx.random.uniform(shape=x.shape) < 1 / args.seg).astype(mx.int32), axis=1
    )
    ...
    for name, tile, topk, combine in variants:
        sa._KEY_TILE = tile
        sa._TOPK_ENABLED = topk
        moe._COMBINE_ENABLED = combine
