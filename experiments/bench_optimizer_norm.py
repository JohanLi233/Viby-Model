...
def main():
    ...
    g = (mx.random.normal(shape) * .01).astype(mx.bfloat16)
    m = mx.zeros_like(p)
    lr = mx.array(.002, mx.bfloat16)
    mx.eval(p, g, m, lr)
    native = mx.compile(_sinkhorn_body(.95, .18, 11, 1e-20, .001, args.cols))
    fast = mx.compile(_sinkhorn_body(.95, .18, 11, 1e-20, .001, args.cols, True))
    a, am = native(p, g, m, lr)
    b, bm = fast(p, g, m, lr)
    delta = b.astype(mx.float32) - a.astype(mx.float32)
    numerical = dict(kind="numerical", shape=shape, max_abs=float(mx.max(mx.abs(delta))),
    ...
    print(json.dumps({k: result[k] for k in ("paired_block_median_B_over_A", "aa_end_over_start_abs_rel")}), flush=True)
    print("median seconds", result["A"]["median_s"], result["B"]["median_s"], flush=True)
