...
def chunked(N):
    ...
    def run():
        ...
        prev = mx.concatenate(
            [mx.zeros((B, 1, W, D), dtype=kv.dtype), kc[:, :-1]], axis=1
        )
        kb = mx.concatenate([prev, kc], axis=2)  # [B,C,2W,D]
        kb = mx.concatenate(
            [kb[:, :, None], mx.broadcast_to(comp[:, None, None], (B, C, 1, N, D))],
            axis=3,
        )
        ...
        m = mx.concatenate(
            [
                mx.broadcast_to(win[None, None, None], (B, C, 1, W, 2 * W)),
                cmask[:, :, None],
            ],
            axis=-1,
        )
        o = mx.fast.scaled_dot_product_attention(
            qc, kb, kb, scale=D**-0.5, mask=m.reshape(B * C, 1, W, 2 * W + N)
        )
        mx.eval(o)
...
for name, N in modes:
    ...
    b = bench(chunked(N))
    print(
        "  %-8s N=%4d  稠密 S=%4d %6.2f/%6.2f ms  分块 S=%4d %6.2f/%6.2f ms"
        % (name, N, T + N, a[0], a[1], 2 * W + N, b[0], b[1]),
        flush=True,
    )
