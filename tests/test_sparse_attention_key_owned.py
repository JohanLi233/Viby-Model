...
def test_one_key_value_path_writes_every_dimension(heads, dim, score_path):
    ...
    (got,) = sa._key_owned_kernels()[-2](
        inputs=[
            q,
            window,
            mx.zeros((1, 0, dim), q.dtype),
            mx.full((1, 1, heads), np.log(4), mx.float32),
            g,
            delta,
            mx.zeros((1,), mx.int32),
            mx.zeros((0,), mx.int32),
            mx.zeros((0,), mx.int32),
            mx.zeros((1, 1), mx.int32),
            mx.ones((1, 1), mx.bool_),
            mx.array([1, 1, 0], mx.uint32),
    ...
    expected = (
        mx.sum(
            g.astype(mx.float32) - delta[..., None] * q.astype(mx.float32), axis=(1, 2)
        )
        * 0.25
    )
    mx.eval(got, expected)
    np.testing.assert_array_equal(
        np.asarray(got.astype(mx.float32)), np.asarray(expected)
    )
...
def test_key_owned_vjp_matches_query_owned_and_eager(
    monkeypatch, dtype, batch, seq, keys, dim, window
):
    mx.set_default_device(mx.gpu)
    ...
    y, grads = _run_vjp(
        sa.indexed_attention, q, w, c, sinks, vis, seg, pad, W, scale, cot
    )
    mx.eval(y, grads)
    ...
    yeager, geager = _run_vjp(
        eager_indexed_attention, q, w, c, sinks, vis, seg, pad, W, scale, cot
    )
    mx.eval(yref, gref, yeager, geager)
    np.testing.assert_array_equal(
        np.asarray(y.astype(mx.float32)), np.asarray(yref.astype(mx.float32))
    )
    for name, got, ref, eager in zip(("dq", "dw", "dc", "ds"), grads, gref, geager):
        actual, expected, exact = [
            np.asarray(x.astype(mx.float32)) for x in (got, ref, eager)
        ]
        if name in ("dq", "ds"):
...
def _assert_pool_close(actual, expected, dtype, name):
    ...
    np.testing.assert_allclose(
        actual,
        expected,
        rtol=1e-2 if bf16 else 2e-3,
        atol=1e-2 if bf16 else 2e-3,
        err_msg=name,
    )
...
def test_compressed_hot_key_parts_cover_all_occurrences(heads, dim):
    ...
    rp, eq, es = sa._compressed_occurrence_csr(
        mx.array(idx), mx.ones((t,), mx.int32), 1, t, n
    )
    q = mx.zeros((1, t, heads, dim), mx.float16)
    (got,) = sa._key_owned_kernels()[-2](
        inputs=[
            q,
            mx.zeros((1, t, dim), q.dtype),
            mx.zeros((1, n, dim), q.dtype),
            mx.full((1, t, heads), np.log(4), mx.float32),
            mx.ones_like(q),
            mx.zeros((1, t, heads), mx.float32),
            rp,
            eq,
            es,
            mx.zeros((1, t), mx.int32),
            mx.zeros((1, t), mx.bool_),
            mx.array([1, t, n], mx.uint32),
            mx.array([1.0], mx.float32),
        ],
        template=[
            ("T", q.dtype),
...
def _eager_occurrence_attention(q, w, c, indices, lengths, seg, pad, sinks, W, scale):
    ...
    valid_w = (
        (pos[None] >= 0)
        & pad[batch, safe_pos[None]]
        & (seg[batch, safe_pos[None]] == seg[:, :, None])
    )
    ...
    scores = (
        mx.sum(q.astype(mx.float32)[:, :, :, None, :] * keys[:, :, None, :, :], axis=-1)
        * scale
    )
    scores = mx.where(valid[:, :, None, :], scores, -float("inf"))
    logits = mx.concatenate(
        [scores, mx.broadcast_to(sinks.reshape(1, 1, h, 1), (b, t, h, 1))], axis=-1
    )
    p = mx.softmax(logits, axis=-1)[..., :-1]
    return (
        mx.sum(p[..., None] * keys[:, :, None], axis=-2)
        .astype(q.dtype)
        .transpose(0, 2, 1, 3)
    )
...
def test_duplicate_selection_vjp_and_shared_primal(monkeypatch, dtype):
    ...
    def run(key_owned, eager=False, compiled=False):
        ...
        def fn(a, window, compressed, s):
            def layer(qq):
                if eager:
                    return _eager_occurrence_attention(
                        qq, window, compressed, indices, lengths, seg, pad, s, W, scale
                    )
    ...
    for got, expected in zip(grads, gc):
        np.testing.assert_array_equal(
            np.asarray(got.astype(mx.float32)), np.asarray(expected.astype(mx.float32))
        )
    ...
    for output in (yc, yr, ye):
        np.testing.assert_allclose(
            np.asarray(y[0].astype(mx.float32)),
            np.asarray(output[0].astype(mx.float32)),
            rtol=5e-2,
            atol=2e-1,
        )
