...
from model.moe import Expert, MoEFeedForward, MoEGate, expert_act, softplus, update_expert_bias
...
def test_sqrtsoftplus_score_formula():
    ...
    assert np.allclose(np.asarray(got), want, atol=1e-5), np.abs(np.asarray(got) - want).max()
    assert float(mx.min(got)) > 0.0
    # softplus 本身：log(1+e^x)
    v = mx.array([-3.0, 0.0, 2.5])
    mx.eval(softplus(v))
    assert np.allclose(np.asarray(softplus(v)), np.logaddexp(np.asarray(v), 0), atol=1e-6)
...
def test_bias_changes_selection_but_not_weights():
    ...
    scores0 = np.asarray(s0)[0]
    missed = [e for e in range(cfg.n_routed_experts) if e not in np.asarray(idx0)[0].tolist()]
    e = missed[0]
...
def test_router_bias_in_parameters_but_frozen():
    ...
    assert all(k not in trainable for k in bias_keys), "router.bias 必须 freeze（不进优化器）"
    # 也不出现在梯度树里
...
def test_stacked_experts_gate_up_layout():
    ...
    mx.eval(gate, up)
    for e in (0, 3):
        want_gate = np.asarray(x) @ np.asarray(ffn.experts.gate_up_w[e])[:cfg.moe_inter_dim].T
        want_up = np.asarray(x) @ np.asarray(ffn.experts.gate_up_w[e])[cfg.moe_inter_dim:].T
...
def test_shared_expert_is_always_added():
    ...
    assert np.allclose(np.asarray(total), np.asarray(routed) + np.asarray(shared), atol=1e-5)
    # 把共享专家清零 → 输出只剩路由专家
...
def test_load_stats_count_topk_assignments():
    ...
    assert np.allclose(np.asarray(mx.sum(out.moe_loads, axis=-1)),
...
def test_update_moe_biases_end_to_end():
    ...
    model = build(cfg_tiny())        # 本用例会改公开偏置，用新模型避免污染缓存
    cfg = model.config
    mx.random.seed(68)
    ...
    # 每个专家 ±rate（负载恰好等于均值时 sign=0 → 不动）
    assert (delta <= cfg.bias_update_rate + 1e-9).all()
    assert delta.max() == pytest.approx(cfg.bias_update_rate, abs=1e-9)
