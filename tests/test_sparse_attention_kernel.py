...
def eager_indexed_attention(
    q, window, compressed, visible, segment_ids, pad_mask, sinks,
    window_size, softmax_scale,
):
    ...
    if n > 0:
        ...
        scores_c = mx.sum(qf[:, :, :, None, :] * cf[:, None, None, :, :], axis=-1) * scale
...
def _inputs(b, t, n, head_dim, window, seed, dtype=mx.bfloat16):
    ...
    compressed = mx.random.normal((b, n, head_dim)).astype(dtype) if n else mx.zeros((b, 0, head_dim), dtype)
    if n:
...
@pytest.mark.parametrize("seq,keys,dim,window", [
    (1, 1, 128, 32),
    (8, 4, 128, 8),
    (17, 17, 64, 16),
    (4, 0, 128, 4),
...
def test_indexed_attention_forward_matches_eager(seq, keys, dim, window):
    ...
    q, wkv, ckv, vis, seg, pad, sinks, W, scale = _inputs(1, seq, keys, dim, window, seed=3)
