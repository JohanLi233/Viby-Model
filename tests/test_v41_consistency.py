...
def test_prefill_equals_decode_with_ratio2_and_reindex():
    ...
    model = mix_model()
    cfg = model.config
    assert 2 in cfg.compress_ratios and "reindex" in [
        cfg.layer_mode(i) for i in range(cfg.n_layers)
    ]
    mx.random.seed(31)
...
def test_packed_segments_isolate_documents():
    ...
    seg = mx.concatenate(
        [mx.zeros((1, 5), dtype=mx.int32), mx.ones((1, 11), dtype=mx.int32)], axis=1
    )
...
def test_segment_and_pad_together():
    ...
    am = mx.concatenate(
        [mx.ones((1, 6), dtype=mx.int32), mx.zeros((1, 2), dtype=mx.int32)], axis=1
    )
...
def test_continuous_batch_decode_with_unequal_lengths(l1, l2):
    ...
    out = model(
        tok[:, None],
        start_pos=mx.array([l1, l2], dtype=mx.int32),
        cache=bc,
        decode=True,
        use_mtp=False,
    )
...
def test_batch_decode_needs_engram_prev_tokens():
    ...
    good = model(
        ...
        cache=bc,
        decode=True,
        prev_tokens=mx.concatenate([i1[:, l1 - w : l1], i2[:, l2 - w : l2]], axis=0),
        use_mtp=False,
    )
    mx.eval(good.logits)
