...
用法: uv run tests/test_attn_fused.py
"""
...
def run_large_qk():
    """训练中后期 QK 分数可超过 f32 exp 上限（~89）。手写 fwd 必须
    用 online softmax，输出与梯度都有限，并贴近 f32 朴素 softmax。"""
    B, H, T, Dk, Dv = 1, 2, 64, 128, 96
    mx.random.seed(0)
    gain = 5.0
    q = (mx.random.normal((B, H, T, Dk)) * gain).astype(mx.bfloat16)
    k = (mx.random.normal((B, H, T, Dk)) * gain).astype(mx.bfloat16)
    v = (mx.random.normal((B, H, T, Dv)) * 0.5).astype(mx.bfloat16)
    C = mx.random.normal((B, H, T, Dv)) * 0.5
    scale = Dk**-0.5
    mx.eval(q, k, v, C)

    new_o = flash_sdpa(q, k, v, scale=scale, mask="causal", v_dim=Dv)
    ref_o = naive_sdpa(q, k, v, scale, "causal")
    mx.eval(new_o, ref_o)
    finite = bool(mx.all(mx.isfinite(new_o.astype(mx.float32))).item())
    check("大 QK 前向有限", finite)
    if finite:
        r_fwd = relmax(new_o, ref_o)
        check("大 QK 前向 vs f32 softmax", r_fwd < 8e-2, f"  relmax={r_fwd:.2e}")

    def loss_naive(q_, k_, v_):
        return (naive_sdpa(q_, k_, v_, scale, "causal") * C).sum()

    def loss_new(q_, k_, v_):
        o = flash_sdpa(q_, k_, v_, scale=scale, mask="causal", v_dim=Dv)
        return (o.astype(mx.float32) * C).sum()

    ln, gn = mx.value_and_grad(loss_naive, argnums=(0, 1, 2))(q, k, v)
    lf, gf = mx.value_and_grad(loss_new, argnums=(0, 1, 2))(q, k, v)
    mx.eval(ln, gn, lf, gf)
    gfin = all(bool(mx.all(mx.isfinite(g.astype(mx.float32))).item()) for g in gf)
    check("大 QK 梯度有限", gfin)
    if gfin:
        tags = ("dq", "dk", "dv")
        rels = [relmax(a, b) for a, b in zip(gf, gn)]
        ok = all(r < 0.15 for r in rels)
        detail = "  " + ", ".join(f"{t}={r:.2e}" for t, r in zip(tags, rels))
        check("大 QK 梯度 vs f32 naive", ok, detail)
