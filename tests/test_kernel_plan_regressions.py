...
@pytest.mark.parametrize(
    "rows,dim", [(1023, 64), (1024, 64), (1027, 160), (1024, 4096)]
)
def test_grouped_hc_preserves_all_token_gradients(monkeypatch, rows, dim):
    ...
    fwd, bwd = hc._kernels()
    _, h, r = fwd(
        inputs=[x, p, w, mx.array([1e-6])],
        template=[("T", x.dtype), ("D", dim)],
        grid=(128, rows, 1),
        threadgroup=(128, 1, 1),
    ...
    np.testing.assert_array_equal(
        results[1][2], np.asarray(expected.astype(mx.float32))
    )
...
def test_indexer_empty_tiles_keep_reachable_scores_and_gradients(rows, keys):
    ...
    reach = mx.broadcast_to(
        (mx.arange(keys) // 16 % 2 == 1)[None, None], (1, rows, keys)
    )
    out, grads = mx.vjp(ix.indexer_score, [q, k, w, reach], [mx.ones(reach.shape)])
    mx.eval(out, grads)
    np.testing.assert_array_equal(
        np.asarray(out[0]), np.asarray(mx.where(reach, 256.0, -1e30))
    )
    ...
    np.testing.assert_array_equal(
        np.asarray(grads[1][0].astype(mx.float32)),
        np.asarray(
            mx.broadcast_to(expected_k, (keys, 64))
            .astype(mx.float16)
            .astype(mx.float32)
        ),
    )
...
def test_token_owned_combine_fp32_reference(routes, dim, rows, dtype):
    ...
    def ref(a, w):
        return mx.sum(
            a[inv].astype(mx.float32).reshape(rows, routes, dim)
            * w.reshape(rows, routes, 1),
            axis=1,
        ).astype(dtype)
...
def test_packed_selection_compiles_with_dynamic_lengths():
    ...
    for score in ([1.0, 2.0, 3.0, 4.0], [1e10] * 4):
        offsets, _, (flat, lengths) = fn(
            mx.array([[score]], mx.float32), mx.ones((1, 1, 4), mx.bool_)
        )
...
def test_forward_tile32_uses_bounded_tile16_backward(dim):
    ...
    yr, gr = _run_vjp(
        eager_indexed_attention, q, w, c, sink, visible, seg, pad, W, scale, cot
    )
