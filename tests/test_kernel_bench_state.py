...
from experiments.kernel_bench_utils import (
    abba_blocks, restore_train_state, snapshot_train_state,
)
...
def test_aa_keeps_both_slot_groups():
    calls = []
    result = abba_blocks(lambda: calls.append("same"), lambda: calls.append("same"),
                         warmup=0, block_iters=2, n_blocks=1)
    assert len(calls) == 8
