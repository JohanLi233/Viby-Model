...
def test_square_sum(shape, dtype):
    ...
    for axis in (0, 1):
        ...
        np.testing.assert_allclose(
            np.array(actual), np.array(expected), rtol=2e-6, atol=1e-5
        )
...
def test_sinkhorn_all_iterations_and_state(dtype):
    ...
    refs = (p, m)
    opts = (p, m)
    native = mx.compile(_sinkhorn_body(0.95, 0.18, 11, 1e-20, 0.001, 128))
    fast = mx.compile(_sinkhorn_body(0.95, 0.18, 11, 1e-20, 0.001, 128, True))
    for _ in range(3):
        refs = native(refs[0], g, refs[1], lr)
        opts = fast(opts[0], g, opts[1], lr)
        mx.eval(refs, opts)
        np.testing.assert_array_equal(
            np.array(refs[1].astype(mx.float32)), np.array(opts[1].astype(mx.float32))
        )
        ...
        assert float(mx.sqrt(mx.mean(delta * delta))) < (
            2e-4 if dtype == mx.bfloat16 else 2e-7
        )
...
def test_gradient_norm_tails_and_precision(size, dtype):
    ...
    np.testing.assert_allclose(
        float(gradient_square_sum(x)), float(expected), rtol=2e-6
    )
    if size:
