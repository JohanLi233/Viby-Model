...
def create_mixed_optimizer(model, args, ddp=False, training_type="pretrain"):
    ...
    if training_type == "sft":
        # SFT的学习率倍数设置 - 使用较小的学习率防止灾难性遗忘
        embed_lr_mult = 0.1  # 嵌入层需要特别小心，避免破坏预训练的词汇表征
        scalar_lr_mult = 0.3  # 标量参数（如LayerNorm）可以适度调整
        weight_decay = 0.01  # SFT使用较小的权重衰减
    else:
        # 预训练的学习率倍数设置 - 从零开始学习，需要较大学习率
        embed_lr_mult = (
            1.0  # 嵌入层需要充分学习词汇表征（由于权重绑定，输出头共享此参数）
        )
...
def log_training_progress(
    ...
    Logger(
        "Epoch:[{}/{}]({}/{}) loss:{:.3f} lr:{:.2e} step/s:{:.2f} tokens/s:{:.0f} eta:{}min".format(
            epoch + 1,
            args.epochs,
            step,
            iter_per_epoch,
            ...
            steps_per_sec,
            tokens_per_sec,
            (iter_per_epoch - step - 1) / steps_per_sec / 60,
        )
    )
