...
def pointer_batch(seed, batch_size, nodes, depth, rounds, slots):
    ...
    inputs = np.concatenate([table + 1, np.full((batch_size, 1), nodes + 1),
                             start[:, None] + 1, np.full((batch_size, 1), nodes + 2 + depth)], axis=1).astype(np.int32)
    node = start.copy()
    ...
    address = np.stack(trajectory[:rounds], axis=1) if rounds else np.empty((batch_size, 0), dtype=np.int32)
    # Once the requested computation is complete, no further address is required.
    if rounds > depth:
        address[:, depth:] = -1
    states = np.stack([trajectory[min(r, depth)] + 1 for r in range(rounds + 1)], axis=1)
    labels = np.zeros_like(inputs)
    labels[:, -1] = answer
    mask = np.zeros_like(inputs, dtype=np.float32)
    mask[:, -1] = 1
    return {
        "input_ids": mx.array(inputs), "labels": mx.array(labels), "loss_mask": mx.array(mask),
        "answer": mx.array(answer), "prefix_length": inputs.shape[1],
        "targets": {"address": mx.array(np.repeat(address[..., None], slots, axis=-1)),
        ...
        "heldout_results": mx.array(((states - 1) == start[:, None]).astype(np.int32)[..., None]),
